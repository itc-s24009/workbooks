...
import { shareWorkbook } from "@/actions/shareActions"; // シェア機能をインポート
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[200] p-4 text-left">
      <form onSubmit={handleShare} className="bg-gray-800 border border-gray-700 rounded-3xl w-full max-w-sm shadow-2xl p-8 flex flex-col">
        ...
        <p className="text-center text-xs text-gray-500 mb-6 font-mono break-all">{item.name}</p>
        
        <div className="mb-6">
          <label className="text-xs font-bold text-gray-500 uppercase tracking-widest block mb-2">送信先のメールアドレス</label>
          <input 
            type="email" 
            value={email}
        ...
        <div className="flex gap-4">
          <button type="button" onClick={onClose} className="flex-1 bg-gray-700 text-gray-300 font-bold py-3 rounded-xl hover:bg-gray-600 transition">中止</button>
          <button 
            type="submit" 
            disabled={isProcessing}
            className="flex-1 bg-blue-600 text-white font-black py-3 rounded-xl disabled:opacity-50 hover:bg-blue-500 transition"
          >
            {isProcessing ? "送信中..." : "送る (Enter)"}
          </button>
        </div>
      </form>
    </div>
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[150] p-4">
      <div className="bg-gray-800 border border-gray-700 rounded-3xl w-full max-w-md flex flex-col h-[70vh] shadow-2xl">
        <div className="p-6 border-b border-gray-700">
          ...
          <p className="text-gray-400 text-sm mt-1"><span className="font-bold text-blue-500">Target:</span> {item.name}</p>
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-1 bg-gray-900/50">
          {currentPath !== null && (
            <button onClick={goBack} className="w-full text-left p-4 hover:bg-gray-700 rounded-xl text-blue-400 font-bold transition flex items-center gap-2"><span>⇠</span> 親フォルダに戻る</button>
          )}
          {loading ? <p className="p-10 text-center text-gray-500 animate-pulse">読み込み中...</p> : 
           dirs.filter(d => d.id !== item.id).map(d => (
            <button key={d.id} onClick={() => enterDir(d.id, d.name)} className="w-full text-left p-4 hover:bg-gray-700 rounded-xl flex items-center gap-3 transition group">
              <FolderIcon /> <span className="text-gray-200 font-medium group-hover:text-white">{d.name}</span>
            </button>
          ))}
        ...
        <div className="p-6 bg-gray-800 border-t border-gray-700 grid grid-cols-2 gap-4">
          <button onClick={onClose} className="py-4 rounded-xl bg-gray-700 text-gray-300 font-bold hover:bg-gray-600 transition">やめる</button>
          <button onClick={handleMoveExecute} className="py-4 rounded-xl bg-blue-600 text-white font-black hover:bg-blue-500 transition shadow-lg shadow-blue-900/20">移動 (Enter)</button>
        </div>
      </div>
    </div>
  );
...
function ItemRow({ item }: { item: Item }) {
  ...
  const [isSharing, setIsSharing] = useState(false); // シェア用フラグ
  ...
  return (
    <div className="flex items-center justify-between border border-gray-700 bg-gray-800 p-4 rounded-xl shadow-sm hover:bg-gray-750 transition-all text-white">
      <Link href={item.type === 'directory' ? `/directory/${item.id}` : `/workbook/${item.id}`} className="flex-grow flex items-center gap-4 group">
      ...
      <div className="relative" ref={menuRef} onMouseLeave={() => setIsMenuOpen(false)}>
        ...
        {isMenuOpen && (
          <div className="absolute right-0 top-10 w-40 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl z-30 overflow-hidden">
            ...
            <button onClick={() => { setIsMoving(true); setIsMenuOpen(false); }} className="block w-full text-left px-4 py-3 text-sm hover:bg-gray-700 border-t border-gray-700">移動</button>
            
            {/* ▼ シェア機能の追加（問題集の場合のみ表示） ▼ */}
            {item.type === 'workbook' && (
              <button 
                onClick={() => { setIsSharing(true); setIsMenuOpen(false); }} 
                className="block w-full text-left px-4 py-3 text-sm hover:bg-gray-700 text-blue-400 border-t border-gray-700"
              >
                シェア
              </button>
            )}
            
            <button onClick={() => { setIsDeleting(true); setIsMenuOpen(false); }} className="block w-full text-left px-4 py-3 text-sm text-red-500 hover:bg-gray-700 border-t border-gray-700">削除</button>
          </div>
        )}
      </div>
      ...
      {isEditing && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[200]">
          <form onSubmit={async (e) => { e.preventDefault(); await updateItem(item.id, item.type, { name: newName, description: newDesc }); setIsEditing(false); }} className="bg-gray-800 border border-gray-700 p-8 rounded-3xl w-full max-w-sm">
      ...
      {isDeleting && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[200] p-4 text-center">
          <div className="bg-gray-800 border border-gray-700 p-10 rounded-3xl max-w-sm w-full">
             ...
             <p className="text-gray-400 text-sm mb-8 leading-relaxed">「{item.name}」に関連する全データが削除されます。</p>
             <button onClick={async () => await deleteItem(item.id, item.type)} className="bg-red-600 hover:bg-red-500 text-white font-black px-10 py-3 rounded-2xl w-full transition shadow-xl">今すぐ削除</button>
             <button onClick={() => setIsDeleting(false)} className="text-gray-500 text-xs mt-6 block w-full hover:underline font-bold tracking-widest uppercase">キャンセル</button>
          </div>
        </div>
      )}
    </div>
